...
if __name__ == '__main__':
  experiment = f"{os.path.basename(os.path.dirname(Path(__file__)))}_prompteng"
  ...
  for job_idx, (n_tasks, device,           
    ...
    for task_idx in range(n_tasks):
      ...
      logdir = f"{base_dir}/databricks_toxicity_gpt2_llama-prompt-eng_peft/ppo{batch_size[task_idx]}_gpt2_kl{init_kl_coef[task_idx]}_bleu{bleu_reward_coef[task_idx]}_cossimemb{cossimemb_reward_coef[task_idx]}_ent{ent_reward_coef[task_idx]}_textsim{textual_sim_reward_coef[task_idx]}_gebrish{giberish_penalty_coef[task_idx]}_targdiv{target_sim_div_reward_coef[task_idx]}/{suffix}"
      config = maybe_escape_quote(json.dumps({
          ...
          "method.giberish_model_device": "cpu",
          "method.cossimemb_model_device": "cpu",
          "train.batch_size": batch_size[task_idx],
          "train.logging_dir": logdir,
          "train.checkpoint_dir": logdir,
          "train.seed": seed[task_idx],
          
          # Adjust the following values baesd on machines
          "train.minibatch_size": 32,
          "method.chunk_size": 2,
          "method.reward_model_device_offset": reward_model_device_offset,
