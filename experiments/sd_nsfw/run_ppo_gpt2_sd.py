...
if __name__ == '__main__':
  ...
  algos_scripts = [
    {"script": "CUDA_VISIBLE_DEVICES={gpus} python3 ppo_gpt2_sd_peft_nsfw.py",},
  ]
  ...
  for job_idx, (n_tasks, device,           
    ...
    for task_idx in range(n_tasks):
      ...
      logdir = f"{base_dir}/sd_nsfw_gpt2/ppo{batch_size[task_idx]}_gpt2_kl{init_kl_coef[task_idx]}_bleu{bleu_reward_coef[task_idx]}_cossimemb{cossimemb_reward_coef[task_idx]}_ent{ent_reward_coef[task_idx]}_textsim{textual_sim_reward_coef[task_idx]}_gebrish{giberish_penalty_coef[task_idx]}_targdiv{target_sim_div_reward_coef[task_idx]}/{suffix}"
