...
@dataclass
class CausalLMOutputWithEIPOValue(ModelOutput):
    loss: Optional[torch.FloatTensor] = None
    logits: Optional[torch.FloatTensor] = None
    past_key_values: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
    hidden_states: Optional[Tuple[torch.FloatTensor]] = None
...
def make_policy_branch(base_model, num_policy_layers_unfrozen):
    outsize = base_model.lm_head.out_features
    policy_head = make_head(hf_get_hidden_size(base_model.config), outsize)
    if num_policy_layers_unfrozen == 0:
        return policy_head
    config = base_model.config
    branch_class = hf_get_branch_class(config)
    policy_branch = branch_class(base_model, num_layers_unfrozen=num_policy_layers_unfrozen, frozen=False)
    policy_branch.lm_head = policy_head
...
class AutoModelForCausalLMWithValueHead(PreTrainedModelWrapper):
    ...
    def __init__(
        ...
        self.v_head = self.v_head.to(base_model.device)
...
class AutoModelForCausalLMWithEIPOValueHead(AutoModelForCausalLMWithValueHead):
    ...
    _auto_model_parent_class = transformers.AutoModelForCausalLM
    _supported_modules = ["v_head", "v_aux_head", "pi_aux_head"]
    _supported_args = ["peft_config", "num_value_layers_unfrozen"]
    ...
    def __init__(
        ...
        super().__init__(base_model, peft_config=peft_config, quantization_config=quantization_config)
        self.num_value_layers_unfrozen = num_value_layers_unfrozen
        self.v_aux_head = make_value_branch(base_model, num_value_layers_unfrozen)
        self.v_aux_head = self.v_aux_head.to(base_model.device)
        self.pi_aux_head = make_policy_branch(base_model, num_value_layers_unfrozen)
        self.pi_aux_head = self.pi_aux_head.to(base_model.device)
    ...
    def forward(
        ...
        # EIPO-TODO: figure out what's going on in the following code
        if self.peft_type and ignore_peft_adapter:
            if "LORA" in self.peft_type:
                # For LORA, temporarily disable the adapter
                lora_model = self.base_model.base_model
    ...
    def generate(self, *args, **kwargs) -> Union[ModelOutput, torch.LongTensor]:
        # EIPO-TODO: generate from both or allow the arguments
        return self.base_model.generate(*args, **kwargs)
