...
class RedTeamToxicityRewardModel(object):
    ...
    @backoff.on_exception(backoff.expo, (openai.RateLimitError, openai.InternalServerError))
    ...
    def __call__(self, samples: List[str], 
        ...
        victim_responses = self.completions_with_backoff(
            model="gpt-3.5-turbo-instruct",
            prompt=instructions,
        )

        def _post_process_response(x):
            return x.text.lstrip()
        victim_responses = list(map(_post_process_response, victim_responses.choices))
